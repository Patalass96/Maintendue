...
class AssociationController extends Controller
{
    ...
    public function completeProfile(Request $request)
    {
        ...
        if ($request->filled('item_type') || $request->filled('needs_description')) {
            ...
            // $donation->association_id = $association->id;
            // $donation->title = $request->filled('request_title') 
            //     ? $request->input('request_title') 
            //     : 'Besoins de ' . $association->legal_name;
            // $donation->description = $request->input('needs_description', '');
            // $donation->item_type = $request->input('item_type');
            // $donation->school_level = $request->input('school_level');
            // $donation->quantity = $request->input('quantity', 1);
            // $donation->condition = $request->input('condition');
