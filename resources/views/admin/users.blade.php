        ...
        <div class="admin-filters mb-20">
            <div class="filter-row">
                <div class="search-box">
                    ...
                    <input type="text" id="userSearch" placeholder="Rechercher par nom, email, téléphone..."
                        class="search-input">
                </div>
            ...
            <table class="admin-table" id="usersTable">
                ...
                <tbody>
                    @foreach ($users as $user)
                        @php
                            // Détermine l'icône et le badge selon le rôle
$roleIcon = '';
$roleClass = '';
$roleText = '';
...
switch ($user->role) {
    case 'admin':
        ...
        break;
    case 'association':
        $roleIcon = 'fa-hands-helping';
        $roleClass = 'association';
...
$statusText = $user->is_active ? 'Actif' : 'Inactif';
                        ...
                        <tr data-user-id="{{ $user->id }}" data-status="{{ $statusClass }}"
                            data-role="{{ $user->role }}">
                            <td>
                                <div class="user-info">
                                    <div class="user-avatar role-{{ $roleClass }}">
                            ...
                            <td>
                                <div class="date-info">
                                    ...
                                    <small>Il y a {{ $user->created_at->diffForHumans() }}</small>
                                </div>
                            </td>
                            <td>
                                <div class="date-info">
                                    @if ($user->last_login_at)
                                        ...
                                        <small>Il y a {{ $user->last_login_at->diffForHumans() }}</small>
                                    @else
                                        <div>Jamais</div>
                            ...
                            <td>
                                <div class="table-actions">
                                    <a href="{{ route('admin.users.show', $user->id) }}" class="btn-icon btn-view"
                                        title="Voir">
                                        <i class="fas fa-eye"></i>
                                    </a>

                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="btn-icon btn-warning"
                                        title="Modifier">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    @if ($user->role !== 'admin' && $statusClass !== 'suspended')
                                        <form action="{{ route('admin.users.suspend', $user->id) }}" method="POST"
                                            style="display: inline;">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn-icon btn-danger" title="Suspendre"
                                                onclick="return confirm('Suspendre cet utilisateur ?')">
                                                <i class="fas fa-ban"></i>
                                            </button>
                                        </form>
                                    @elseif($statusClass === 'suspended')
                                        <form action="{{ route('admin.users.activate', $user->id) }}" method="POST"
                                            style="display: inline;">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn-icon btn-success" title="Activer"
                                    ...
                                    @if ($user->role !== 'admin')
                                        <form action="{{ route('admin.users.promote', $user->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn-icon btn-primary" title="Promouvoir admin"
                ...
                @if ($user->status !== 'suspended' && $user->role !== 'admin')
                    <button class="btn-icon btn-danger" title="Suspendre" data-action="suspend"
                        data-id="{{ $user->id }}">
                        <i class="fas fa-ban"></i>
                    </button>
                @elseif($user->status === 'suspended')
                    <button class="btn-icon btn-success" title="Activer" data-action="activate"
                        data-id="{{ $user->id }}">
                        <i class="fas fa-check"></i>
                    </button>
                @endif
                ...
                @if ($user->role !== 'admin')
                    <button class="btn-icon btn-primary" title="Promouvoir admin" data-action="promote"
                        data-id="{{ $user->i }}">
                        <i class="fas fa-star"></i>
                    </button>
                @endif
        ...
        <div class="table-footer">
            <div class="table-info">
                Affichage de 1 à 5 sur 2,450 utilisateurs
            </div>
...
@push('styles')
    <style>
        ...
        .timeline-dot.role-admin {
            background: #8b5cf6;
        }

        .timeline-dot.role-association {
            background: #f59e0b;
        }

        .timeline-dot.role-donor {
            background: #3b82f6;
        }
            ...
            document.querySelectorAll('.table-actions button').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    switch (action) {
                        ...
                        case 'promote':
                            if (confirm(
                                    `Promouvoir l'utilisateur #${userId} en administrateur ?`)) {
