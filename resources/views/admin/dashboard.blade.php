    ...
    <div class="tables-row">
        <div class="table-card">
            ...
            <table class="admin-table">
                ...
                <tbody>
                    @foreach ($pendingAssociations as $item)
                        <tr>
                            <td>{{ $item->legal_name ?? ($item->association_name ?? ($item->association ?? '—')) }}</td>
                            <td>{{ $item->created_at ?? '—' }}</td>
                            <td class="action-btns">
                                <form action="{{ route('admin.validateAssociation', ['id' => $item->id]) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn-validate"
                                        style="padding: 8px 18px; border: 1px solid #e5e7eb; color: #fff; background: green; border-radius: 6px; cursor: pointer; font-weight: 600;">Valider</button>
                                </form>

                                {{-- Pour rejeter une association --}}
                                <form action="{{ route('admin.reject', ['id' => $item->id]) }}" method="POST">
                                    @csrf
